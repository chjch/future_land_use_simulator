...
class Scenario:
    def __init__(self, lu_array):
        ...
        self.border_indices = np.where(self.lu_array == 0)
    ...
    @property
    def border_indices_1d(self):
        return self.border_indices[0] * self.n_col + self.border_indices[1]
    ...
    def plot(self, data=None, ax=None):
        ...
        if ax is None:
            ...
            ax.set_yticks([])
            return ax.pcolor(
                data[::-1], cmap=cmap, norm=norm, dgecolors="k", linewidths=0.5
            )
        else:
            ax.set_xticks([])
            ax.set_yticks([])
            return ax.pcolor(
                data[::-1], cmap=cmap, norm=norm, edgecolors="k", linewidths=0.5
            )
