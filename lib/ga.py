...
def nb_classify(sr, num_class=3):
    ...
    return pd.Series(
        pd.cut(
            sr, bins=breaks, labels=np.arange(1, num_class + 1), include_lowest=True
        ),
        dtype="int",
    )
...
def evaluate(individual):
    ...
    temp_diff = num_lu_diff @ config.temp_slope  # dot product
    if temp_diff == 0:
        temp_constraint = 0
    else:
    ...
    urb_cell_needed = int(
        np.ceil((config.PPL_GROWTH + config.PPL_CURRENT) / config.PPL_PER_URB)
    )
...
def main():
    ...
    # base scenario
    base_scn = Scenario(
        get_current_lu(config.base_df).values.reshape(config.GRID_R, config.GRID_C)
    )
    ...
    while g < config.GEN_NUM:
        ...
        for child1, child2 in zip(offspring[::2], offspring[1::2]):
            if np.random.rand() < config.CXPB:
                ...
                child1[0][base_scn.border_indices_1d] = 0
                child2[0][base_scn.border_indices_1d] = 0
        ...
        for mutant in offspring:
            if np.random.rand() < config.MUTPB:
                ...
                # new
                mutant[0][base_scn.border_indices_1d] = 0
